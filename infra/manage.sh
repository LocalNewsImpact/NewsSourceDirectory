...
# exactly what production is running and nothing needs installing locally. Your
# machine never touches the database — only gcloud credentials are used.
#
...
PROJECT="${PROJECT:-lnic-source-directory}"
...
# routinely contain characters that collide with a chosen delimiter. A pipe does
# not appear in anything we pass.
ARGS="manage.py"
...
gcloud run jobs deploy "$JOB" \
  ...
  --service-account "directory-run@${PROJECT}.iam.gserviceaccount.com" \
