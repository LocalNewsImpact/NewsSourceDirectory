#!/usr/bin/env bash
#
# Apply a SQL script to the shared instance through the Cloud SQL Auth Proxy.
# The proxy authenticates with your gcloud credentials, so nothing is added to
# the instance's authorized networks and no password crosses the network in the
# clear.
#
#   ./infra/sql/apply.sh isolate_directory_role.sql
#
set -euo pipefail

SCRIPT="${1:?usage: apply.sh <file.sql>}"
INSTANCE="${INSTANCE:-mizzou-news-crawler:us-central1:mizzou-db-prod}"
SQL_PROJECT="${SQL_PROJECT:-mizzou-news-crawler}"
PORT="${PORT:-5439}"
DB="${DB:-mizzou}"
USER="${USER_ROLE:-mizzou_user}"

command -v cloud-sql-proxy >/dev/null || {
  echo "cloud-sql-proxy not on PATH — https://github.com/GoogleCloudPlatform/cloud-sql-proxy/releases" >&2
  exit 1
}

cloud-sql-proxy --port "$PORT" "$INSTANCE" >/tmp/csp.log 2>&1 &
PROXY=$!
trap 'kill "$PROXY" 2>/dev/null || true' EXIT

for _ in $(seq 1 30); do
  nc -z 127.0.0.1 "$PORT" 2>/dev/null && break
  sleep 1
done

# The role must be a member of cloudsqlsuperuser to alter database privileges.
PGPASSWORD="$(gcloud secrets versions access latest --secret=db-password --project="$SQL_PROJECT")" \
  psql -h 127.0.0.1 -p "$PORT" -U "$USER" -d "$DB" -v ON_ERROR_STOP=1 -f "$SCRIPT"

# The directory half must run as the directory role against its own database,
# because ownership has moved and the crawler's role can no longer alter it.
if [ "$(basename "$SCRIPT")" = "isolate_directory_role.sql" ]; then
  echo
  echo "== second half, as the directory role =="
  PGPASSWORD="$(gcloud secrets versions access latest --secret=db-password --project=lnic-source-directory)" \
    psql -h 127.0.0.1 -p "$PORT" -U directory -d directory -v ON_ERROR_STOP=1 \
      -f "$(dirname "$SCRIPT")/harden_directory_db.sql"
fi
